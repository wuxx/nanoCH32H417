...
void LCD_Writ_Bus(u8 dat)
{
  ...
  LCD_CS_Set();
    ...
    dummy = SPI_I2S_ReceiveData(SPI2);  // 读取数据清除RXNE标志
	(void)dummy;
	//Delay_Us(3);  // 添加延迟，让LCD有时间处理
	LCD_CS_Set();
#endif
}
...
// 内部函数：发送单字节但不控制CS
static void LCD_SendByte(u8 dat)
{
#if defined(SPI_GPIO)
	u8 i;
	for(i=0;i<8;i++)
...
int lcd_gpio_init()
{
	...
	RCC_HB2PeriphClockCmd(RCC_HB2Periph_GPIOB | RCC_HB2Periph_GPIOD, ENABLE);
    ...
    // 设置SCLK空闲时为高电平 (匹配硬件SPI的CPOL_High)
    LCD_SCLK_Set();
    LCD_CS_Set();
    LCD_MOSI_Set();
    ...
    RCC_HB2PeriphClockCmd( RCC_HB2Periph_AFIO | RCC_HB2Periph_GPIOB | RCC_HB2Periph_GPIOD , ENABLE );
    ...
    SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_Mode0;
...
void LCD_ShowPicture_Fast(u16 x,u16 y,u16 length,u16 width,const u8 pic[])
{
	...
	for(i=0;i<length;i++)
	{
		for(j=0;j<width;j++)
		{
			LCD_Writ_Bus(pic[k*2]);
			LCD_Writ_Bus(pic[k*2+1]);
			k++;
		}
	}
}
